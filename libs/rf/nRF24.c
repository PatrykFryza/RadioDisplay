...
#define NRF24_CSN_HIGH	LL_GPIO_SetOutputPin(NRF24_CSN_GPIO_PORT, NRF24_CSN_PIN);
#define NRF24_CSN_LOW		LL_GPIO_ResetOutputPin(NRF24_CSN_GPIO_PORT, NRF24_CSN_PIN);

#define NRF24_CE_HIGH		LL_GPIO_SetOutputPin(NRF24_CE_GPIO_PORT, NRF24_CE_PIN);
#define NRF24_CE_LOW		LL_GPIO_ResetOutputPin(NRF24_CE_GPIO_PORT, NRF24_CE_PIN);
...
static void nRF24_SendSpi(uint8_t *Data, uint8_t Length){
	for(int i = 0; i < Length; ++i){
		...
		LL_SPI_TransmitData8(NRF24_SPI, Data[i]);
	}
	while(!LL_SPI_IsActiveFlag_TXE(NRF24_SPI));
	while(LL_SPI_IsActiveFlag_BSY(NRF24_SPI));
	LL_SPI_ClearFlag_OVR(NRF24_SPI);
}
