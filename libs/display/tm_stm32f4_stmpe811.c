...
TM_STMPE811_State_t TM_STMPE811_Init(void) {
	...
	/* Reset */
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_SYS_CTRL1, 0x02);
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_SYS_CTRL1, 0x00);
	
	/* Check for STMPE811 Connected */
	i2c_read_bytes(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_REG_CHP_ID_LSB, bytes, 2);
	...
	mode &= ~(0x01); 
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_SYS_CTRL2, mode);
	mode = TM_STMPE811_Read(STMPE811_SYS_CTRL2);
	mode &= ~(0x02); 
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_SYS_CTRL2, mode);

	/* Select Sample Time, bit number and ADC Reference */
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_ADC_CTRL1, 0x49);

	/* Wait for 2 ms */
	LL_mDelay(2);

	/* Select the ADC clock speed: 3.25 MHz */
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_ADC_CTRL2, 0x01);

	/* Select TSC pins in non default mode */  
	mode = TM_STMPE811_Read(STMPE811_GPIO_AF);
	mode |= 0x1E;
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_GPIO_AF, mode);
	...
	*/
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_TSC_CFG, 0x9A);
	/* Configure the Touch FIFO threshold: single point reading */
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_FIFO_TH, 0x01);

	/* Clear the FIFO memory content. */
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_FIFO_STA, 0x01);

	/* Put the FIFO back into operation mode  */
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_FIFO_STA, 0x00);
	/* Set the range and accuracy pf the pressure measurement (Z) : 
	- Fractional part :7 
	- Whole part      :1 
	*/
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_TSC_FRACTION_Z, 0x01);

	/* Set the driving capability (limit) of the device for TSC pins: 50mA */
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_TSC_I_DRIVE, 0x01);

	/* Touch screen control configuration (enable TSC):
	- No window tracking index
	- XYZ acquisition mode
	*/
	i2c_write_byte(STMPE811_I2C, STMPE811_ADDRESS, STMPE811_TSC_CTRL, 0x03);
...
uint8_t TM_STMPE811_Read(uint8_t reg) {
	return i2c_read_byte(STMPE811_I2C, STMPE811_ADDRESS, reg);
}
...
uint8_t i2c_read_byte(I2C_TypeDef* I2Cx, uint8_t slave_addr, uint8_t addr) {
	...
	LL_I2C_TransmitData8(I2Cx, slave_addr);
	...
	LL_I2C_TransmitData8(I2Cx, slave_addr | 0x01);
	while(!LL_I2C_IsActiveFlag_ADDR(I2Cx)){
		LL_mDelay(1);
		if((timeout--) == 0) return 0;
	}
